...
if len(sys.argv) < 5:
    print('Usage: ' + sys.argv[0] + ' dictfile textfile outfile startaddress [--vwf --EU --combo]')
...
while len(sys.argv) > argIndex:
    ...
    if s == '--EU':
        EUText = True
    if s == '--combo':
        suffix = "AGES" if "ages" in outFilename else "SEASONS"
...
split_index_name = "TEXT_OFFSET_SPLIT_INDEX"
define_directive = "define"
defines_filename = 'textDefines'
if suffix:
    split_index_name = split_index_name + "_" + suffix
    define_directive = "redefine"
    defines_filename = suffix.lower() + "_" + defines_filename

definesFile = open('%s/%s.s' % (build_dir, defines_filename), 'w')

definesFile.write('.define %s %s\n\n' % (
    split_index_name, wlahex(textOffsetSplitIndex, 2)
    ))
    ...
    if group.index >= 4:
        for textStruct in group.textStructs:
            for i in range(len(textStruct.names)):
                ...
                definesFile.write('.%s %s %s\n' % (
                    define_directive, textStruct.names[i], wlahex(fullIndex, 4)
                ))
